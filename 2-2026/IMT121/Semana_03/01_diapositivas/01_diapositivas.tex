...
\begin{frame}{De la Inspección Directa al MNA}
    ...
    \begin{itemize}
        \item La formulación estándar asume que todas las ramas están descritas por la Ley de Ohm ($I = G \cdot \Delta V$).
        \item Si una fuente de tensión (independiente o controlada) se conecta entre dos nodos esenciales sin resistencia en serie, su impedancia interna es $R \to 0$ ($G \to \infty$), provocando una \textbf{indeterminación matemática}.
    \end{itemize}
    ...
    \begin{table}[]
        ...
        \begin{tabular}{@{}p{4cm}p{4cm}p{5.5cm}@{}}
            ...
            Fuente entre 2 nodos flotantes & Corriente de rama indefinida por Ohm & Formación de \textbf{Supernodo} + Ec. de Enlace \\
            Fuentes dependientes & Rompe la simetría ($G_{ij} \neq G_{ji}$) & Inclusión de \textbf{variables de control} en la matriz $\mathbf{A}$ \\
            \bottomrule
        \end{tabular}
    \end{table}
\end{frame}
...
\begin{frame}{Tratamiento Físico-Matemático de Supernodos}
    ...
    Superficie cerrada gaussiana que engloba a una fuente de voltaje (independiente o dependiente) conectada entre dos nodos no referenciados, junto con cualquier elemento conectado en paralelo con ella.
    ...
    \begin{block}{Las Dos Ecuaciones Obligatorias del Supernodo}
        \begin{enumerate}
            \item \textbf{Ecuación de Conservación (LCK):} Suma algebraica de las corrientes que \textit{escapan} de la frontera del supernodo: 
            $$ \sum I_{\text{salientes (Nodo 2)}} + \sum I_{\text{salientes (Nodo 3)}} = 0 $$
            \item \textbf{Ecuación de Restricción / Enlace:} Relación constitutiva impuesta por la fuente:
...
\begin{frame}{Fuentes Dependientes como Modelos de Transductores}
    ...
    \begin{center}
        \begin{circuitikz}[scale=0.9, transform shape]
            % VCVS
            \draw (0,3) to[cV, v=$A_v V_x$] (3,3); 
            \node[above, align=center] at (1.5,4) {\textbf{VCVS} \\ \small Tensión controlada por Tensión};
            \node[below, text=blue] at (1.5, 2) {(Control: $V_x$)};
            
            % VCCS
            \draw (6,3) to[cI, i=$g_m V_x$] (9,3); 
            \node[above, align=center] at (7.5,4) {\textbf{VCCS} \\ \small Corriente controlada por Tensión};
            \node[below, text=blue] at (7.5, 2) {(Control: $V_x$)};

            % CCVS
            \draw (0,-0.5) to[cV, v=$R_m I_x$] (3,-0.5); 
            \node[above, align=center] at (1.5,0.5) {\textbf{CCVS} \\ \small Tensión controlada por Corriente};
            \node[below, text=blue] at (1.5, -1.5) {(Control: $I_x$)};

            % CCCS
            \draw (6,-0.5) to[cI, i=$\beta I_x$] (9,-0.5); 
            \node[above, align=center] at (7.5,0.5) {\textbf{CCCS} \\ \small Corriente controlada por Corriente};
            \node[below, text=blue] at (7.5, -1.5) {(Control: $I_x$)};
        \end{circuitikz}
    \end{center}

    \vspace{0.3cm}
    \textbf{Regla de Integración Matricial:} Identificar la variable de control, expresarla estrictamente en función de los potenciales nodales incógnita, y reagrupar términos a la izquierda para mantener $\mathbf{A}\mathbf{x} = \mathbf{b}$.
\end{frame}

% ==========================================
% SLIDE 5a: Ejemplo Integrador (Planteamiento)
% ==========================================
\begin{frame}{Ejercicio Integrador de Ingeniería: Planteamiento}
    \textbf{Circuito de Prueba:} Etapa preamplificadora con fuente independiente $V_s = \qty{10}{\volt}$, resistencias de acoplamiento y una fuente dependiente VCVS ($E_1 = 3V_x$), donde la señal de control es $V_x = V_2$.
    ...
    \begin{columns}
        \begin{column}{0.45\textwidth}
            \begin{center}
                \begin{circuitikz}[scale=0.8, transform shape]
                    ...
                    % Marca de control
                    \draw[<->, dashed, blue] (3.3, 2.8) -- (3.3, 0.2) node[midway, right]{$V_x$};
                \end{circuitikz}
            \end{center}
        \end{column}
        \begin{column}{0.55\textwidth}
            ...
            \begin{itemize}
                \item \textbf{Nodo 1:} Fijo por fuente ideal $\implies V_1 = \qty{10.0}{\volt}$.
...
% ==========================================
\begin{frame}{Ejercicio Integrador de Ingeniería: Resolución LCK}
    ...
    \begin{alertblock}{Sistema Matricial y Solución}
        $$
...
\begin{frame}{Taller de LTspice — Directiva \texttt{.tf} (Transfer Function)}
    La directiva \texttt{.tf} evalúa la función de transferencia, impedancia de entrada y resistencia de salida (Thévenin) sin conectar fuentes de prueba manuales.
    ...
    \begin{columns}
        \begin{column}{0.4\textwidth}
            ...
            \begin{enumerate}
                ...
                \item Asignar \texttt{Value = 3} (Ganancia).
        ...
        \begin{column}{0.6\textwidth}
            ...
            \begin{itemize}
                \item \texttt{Transfer\_function = -2.0000} \\($V_3$ amplifica e invierte $V_1$: $\frac{-20}{10} = -2$).
                \item \texttt{v1\#Input\_impedance = 83.333 ohms}
                \item \texttt{output\_impedance\_at\_v(n003) = 0 ohms} (fuente de tensión ideal).
            \end{itemize}
        \end{column}
    \end{columns}
\end{frame}
...
\begin{frame}{Dinámica Activa en Aula (Tríadas PFI)}
    ...
    Un transductor biomédico/mecatrónico sufre una recalibración térmica que modifica la ganancia de la fuente controlada $E_1$ a $k = 1.5$ ($V_{\text{fuente}} = 1.5 V_x$).
    
    \begin{enumerate}
        \item \textbf{Cálculo Manuscrito:} Plantear la nueva matriz $\mathbf{A}$ y deducir los nuevos valores de $V_2$, $V_3$ y la ganancia $A_v$.
        \item \textbf{Comprobación en Python:} Actualizar el vector y resolver.
    ...
    \begin{alertblock}{Resultados de Autocontrol}
        \begin{itemize}
            ...
            \item Condición de Calidad: $\varepsilon_r (\text{Teoría vs. LTspice}) = 0.00\% \le 5.0\%$
        \end{itemize}
    \end{alertblock}
\end{frame}
